...
public class WaterskinSDST implements IProxy {
    ...
    public void afterTooltip(RenderTooltipEvent.PostText event) {
        ...
        if (QuickConfig.isThirstEnabled() && stack != ItemStack.EMPTY) {
            int thirst = 0;
            float hydration = 0.0F;
            boolean isDrinkable = false;

            if (stack.getItem() instanceof ItemCanteen) {
                thirst = ((ItemCanteen) stack.getItem()).getRestoredThirst();
                hydration = ((ItemCanteen) stack.getItem()).getRestoredHydration();
                isDrinkable = true;
            }
            ...
            if (isDrinkable) {
                ...
                if (ModConfig.client.classicHUDThirst) {
                    mc.getTextureManager().bindTexture(new ResourceLocation("waterskin", "textures/gui/overlay/thirst_overlay_classic.png"));
                } else {
