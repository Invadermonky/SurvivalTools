package com.invadermonky.survivaltools.compat.botania.tiles.subtiles;
...
public class SubTileGryllzalia extends SubTileFunctional implements IAddition, IProxy {
    ...
    */
    ...
    public void onUpdate() {
        super.onUpdate();
        int delay = ConfigHandlerST.integrations.botania.gryllzalia.delay;
        int cost = ConfigHandlerST.integrations.botania.gryllzalia.cost;
        if (!this.supertile.getWorld().isRemote && this.redstoneSignal <= 0 && this.ticksExisted % delay == 0 && this.mana >= cost) {
            ...
            for (EntityPlayer player : players) {
                if (SurvivalToolsAPI.stabilizePlayerTemperature(player, ConfigHandlerST.integrations.botania.gryllzalia.maxCooling, ConfigHandlerST.integrations.botania.gryllzalia.maxHeating)) {
                    this.mana -= cost;
                    did = true;
                }
            }
    ...
    public boolean isEnabled() {
        return ConfigHandlerST.integrations.botania.gryllzalia.enable && SurvivalToolsAPI.isTemperatureFeatureEnabled();
    }
}
